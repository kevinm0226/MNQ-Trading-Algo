...
# =======================
API_KEY = "98fdbc3cace24495b9677288786bdc3c"
ACCOUNT_ID = "51387777"
BASE_URL = "https://demo.ironbeamapi.com"
SYMBOL = "XCME:MNQ.Z25"
TICK_SIZE = 0.25
TICK_VALUE = 0.50
TICKS = 50
...
class IronbeamREST:
    ...
    def cancel_order(self, orderId):
        ...
        url = f"{self.base}/v2/order/{self.account_id}/cancel/{orderId}"
        headers = {"Authorization": f"Bearer {self.token}"}

        try:
            resp = self.session.delete(url, headers=headers, timeout=10)
            resp.raise_for_status()
            print(f"🛑 Order {orderId} canceled successfully")
            return True
...
def strategy(rest, bar):
    ...
    if len(open_orders) > 0:
        ...
        if pl >= 6:
            rest.cancel_order(strategy.active_order_id)
        elif pl <= -6:
            rest.cancel_order(strategy.active_order_id)
        return None

    strategy.active_order_id = rest.place_order(
        SYMBOL,
        "BUY",
        num_contracts,
        "MARKET",
    ...
    # stop_dt = datetime(2025, 10, 29, 16, 0, tzinfo=ZoneInfo("America/New_York"))

    # if now >= stop_dt:
    #     print(
    #         "🛑 Market closed for the day — stopping trading after Oct 29, 2025 4:00 PM ET."
    #     )
    #     return None
