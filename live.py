...
DATA_GATHER_FILE = "data/MNQ_1s_10.27.2025.csv"
...
def strategy(bar):
    ...
    open_orders = get_open_orders(order_status="OPEN")
    if len(open_orders) > 0:
        print("Only one trade brada....")
        return None
...
def get_open_orders(self, order_status: str = "ANY") -> list:
    ...
    try:
        ...
        active_orders = [
            o
            for o in orders
            if o.get("orderStatus", "").upper() in ("OPEN", "WORKING", "PENDING")
        ]
...
def trade_loop(rest):
    while not stop_event.is_set():
        ...
        # Call strategy with rest client
        strategy(rest, bar)
